...
import numpy as np
from utils.data_processing import PreprocessYOLO

# For reading size information ftch.shaperom batches
import struct

class PythonEntropyCalibrator(trt.IInt8EntropyCalibrator2):
    def __init__(self, batch_data_dir, cache_file, input_resolution_yolov6_HW):
        # Whenever you specify a custom constructor for a TensorRT class,
        # you MUST call the constructor of the parent explicitly.
        trt.IInt8EntropyCalibrator2.__init__(self)
        if not os.path.isdir(batch_data_dir):
            raise FileNotFoundError('%s does not exist' % batch_data_dir)
        if len(input_resolution_yolov6_HW) != 2 or input_resolution_yolov6_HW[0] % 32 or input_resolution_yolov6_HW[1] % 32:
            raise ValueError('bad net shape: %s' % str(input_resolution_yolov6_HW))

        self.input_resolution_yolov6_HW = input_resolution_yolov6_HW

        self.cache_file = cache_file
        # Get a list of all the batch files in the batch folder.
        self.batch_files = [os.path.join(batch_data_dir, f) for f in os.listdir(batch_data_dir) if f.endswith('.jpg')]

        # Find out the shape of a batch and then allocate a device buffer of that size.
        self.batch_size = 1
        self.batch_round = len(self.batch_files)
    ...
    def read_batch_file(self, filename):
        ...
        for img_path in filename:
            preprocessor = PreprocessYOLO(self.input_resolution_yolov6_HW)
