...
def build_engine(onnx_file_path, trt_logger, trt_engine_datatype=trt.DataType.FLOAT, batch_size=1, silent=False, dynamic_shapes={},
                calib_data_path=None, calib_file_path=None, input_shape_HW=None):
    ...
    with trt.Builder(trt_logger) as builder, builder.create_network(*EXPLICIT_BATCH) as network, trt.OnnxParser(network,
        ...
        if(trt_engine_datatype == trt.DataType.INT8):
            ...
            from utils.calibrator import PythonEntropyCalibrator
            # builder.int8_mode = True
            config.set_flag(trt.BuilderFlag.GPU_FALLBACK)
            config.set_flag(trt.BuilderFlag.DEBUG)
            config.set_flag(trt.BuilderFlag.INT8)
            config.int8_calibrator = PythonEntropyCalibrator(calib_data_path, cache_file=calib_file_path, input_resolution_yolov6_HW=input_shape_HW)
            print("Builder works on Int8 mode.  Max batch size:{:d}, Max work space size:{:d}.".format(builder.max_batch_size, config.max_workspace_size))
        #  Parse model file
